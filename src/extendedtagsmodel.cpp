...
QVariant ExtendedTagsModel::data(const QModelIndex &index, int role) const
{
    ...
    Tag::Ptr tag = isExtraRow ? m_extraTags.at(row)
                              : m_sourceModel->data(mapToSource(index), TagModel::TagPtrRole).value<Tag::Ptr>();

    switch (role) {
    case TagModel::TagRole:
        return QVariant::fromValue<QObject*>(tag.data());
    case TagModel::TagPtrRole:
...
int ExtendedTagsModel::indexOf(const Tag *tag) const
{
    ...
    for (int i = 0; i < count; ++i) {
        Tag *t = data(index(i, 0), TagModel::TagRole).value<Tag*>();
        if (t == tag)
            return i;
    }
    return -1;
...
{
    return data(index(idx, 0), TagModel::TagPtrRole).value<Tag::Ptr>();
}
